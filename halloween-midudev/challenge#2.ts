...
function battleHorde(zombies: string, humans: string) {
  // Code here
  const zPower = zombies.split("").reduce((acc, prev) => +acc + +prev, 0);
  const hPower = humans.split("").reduce((acc, prev) => +acc + +prev, 0);
  console.log(hPower);
  if (zPower > hPower) return `${zPower - hPower}z`;
  if (zPower < hPower) return `${hPower - zPower}h`;
  return "x";
}
